...
def test_run_scenario_reverts_active_intervention_at_end(tmp_path, monkeypatch):
    from types import SimpleNamespace
    ...
    scenario = SimpleNamespace(name="end_revert_test", interventions=[])

    class AlwaysActiveIntervention:
        def __init__(self):
            self.id = "i2"
            self.intervention_type = "Dummy"
            self.calls = []
    ...
    intervention = AlwaysActiveIntervention()

    monkeypatch.setattr("renewable_fuel_twin.model.apply_scenario", lambda steps, scenario: SimpleNamespace(steps=2, interventions=[], name="end_revert_test"))
    monkeypatch.setattr("renewable_fuel_twin.model.build_interventions", lambda specs: [intervention])
    monkeypatch.setattr("renewable_fuel_twin.model.collect_step_metrics", lambda step, txs: {"step": step})
    monkeypatch.setattr("renewable_fuel_twin.model.summarize_run", lambda step_metrics: {"steps": len(step_metrics)})
    monkeypatch.setattr("renewable_fuel_twin.model.serialize_metrics", lambda *args, **kwargs: None)
    ...
    assert result["events"] == [
        {"step": 0, "event_type": "Dummy", "id": "i2", "transition": "activated"},
        {"step": 2, "event_type": "Dummy", "id": "i2", "transition": "reverted_at_end"},
    ]
    assert intervention.calls == [("apply", 0), ("revert", 2)]
