...
def generate_report(run_result: dict) -> str:
    """
    Generate a formatted Markdown report from a simulation run result.
    
    The input dictionary must contain:
    - "summary": a dict with at least the keys "final_fleet_share_by_fuel" (mapping fuel -> share), "cumulative_emissions" (numeric), and "cumulative_system_cost" (numeric).
    - "events": a sequence of event dicts where each event includes "step" and "event_type" (used to build a short timeline).
    - "scenario": a string describing the scenario.
    
    Parameters:
        run_result (dict): Simulation output containing the keys described above.
    
    Returns:
        str: A multi-section Markdown string including an executive summary (top final fuel and cumulative emissions), scenario description, key outcomes (final fleet shares and cumulative system cost), a short explanation of drivers, a timeline of major events, sensitivity/caveats, and suggested next experiments.
    
    Raises:
        ReportingError: If any required top-level field is missing from `run_result`.
    """
    try:
